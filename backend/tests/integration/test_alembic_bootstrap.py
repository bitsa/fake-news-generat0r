...
def test_T_E1_alembic_ini_present_and_points_at_migrations() -> None:
    ...
    assert re.search(
        r"^\s*script_location\s*=\s*migrations\s*$", text, re.M
    ), "alembic.ini script_location must be 'migrations'"


def test_T_E2_env_py_async_configured() -> None:
    env_py = (REPO_ROOT / "backend" / "migrations" / "env.py").read_text(
        encoding="utf-8"
    )
    assert re.search(
        r"async_engine_from_config|create_async_engine|AsyncEngine", env_py
    ), "env.py must use async SQLAlchemy engine"
    assert (
        "run_sync" in env_py
    ), "env.py must run migrations through connection.run_sync"
...
def test_T_E4_alembic_current_runs_clean() -> None:
    ...
    assert out.returncode == 0, (
        f"alembic current failed: rc={out.returncode}"
        f"\nstdout={out.stdout}\nstderr={out.stderr}"
    )
